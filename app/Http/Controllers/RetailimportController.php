...
use App\Exports\RetailimportExport;
...
class RetailimportController extends Controller
{
    /**
    ...
    {
        $query = Retail_import::orderBy('id', 'desc');
        // dd($query);
        $user = auth()->user();
            ...
            return DataTables::of($reports)
               ...
               ->addColumn('outlet_id', function ($data) {
                ...
                ->addColumn('action', function ($data) {
                    $editRoute = route('retail-import.edit', $data->id);
                    $deleteRoute = route('retail-import.destroy', $data->id);
        ...
        return view('backend.retail.import-list', compact('is_filter', 'full_name'));
        // dd('HI Wholesale');
    ...
    public function create()
    {
        ...
        return view('backend.retail-import.take-photo', compact('customer', 'customers','report','customerType'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('retail-import.index')->with('success', 'File imported successfully.');
    }
    ...
    {
        $report = Retail_import::with('user')->find($id);
        if (!$report) {
    ...
    {
        $report = Retail_import::find($id);
        if (!$report) {
            return redirect()->route('retail-import-import.index');
        }
        ...
        // $customerType = AppHelper::CUSTOMER_TYPE;
        return view('backend.retail.take-photo', compact('report'));
    }
    ...
    public function update(Request $request, $id)
    {
        $report = Retail_import::find($id);
        if (!$report) {
            return redirect()->route('retail-import-import.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('retail-import.index')->with('success', "Report has been updated!");
    }
    ...
    {
        // dd('HI Export');
        return Excel::download(new RetailimportExport(), 'reports_retail_dataimport' . now()->format('Y_m_d_His') . '.xlsx');
    }
    ...
    public function getReports()
    {
        ...
        $query = Retail_import::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            Retail_import::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            Retail_import::whereNull('deleted_at')
