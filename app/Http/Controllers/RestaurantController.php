...
class RestaurantController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Restaurant::with('user', 'CustomerProvince', 'region', 'outlet')->whereDate('created_at', today())->orderBy('id', 'desc');
        $user = auth()->user();
         ...
         * }
        */
        return view('backend.restaurant.index', compact('is_filter', 'full_name', 'reports', 'indexof', 'regions'));
        // dd('HI Wholesale');
    ...
    public function create()
    {
        ...
        return view('backend.restaurant.add', compact('customer', 'customers', 'report', 'customerType', 'regions', 'outlets', 'posms'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('restaurant.index')->with('success', __('Report restaurant has been created successfully.'));
    }
    ...
    */
    ...
    public function show($id)
    {
        $data['report'] = Restaurant::with('user', 'CustomerProvince', 'region', 'outlet', 'posm1', 'posm2', 'posm3')->findOrFail($id);
        return view('backend.restaurant.show', $data);
    }
    ...
    {
        $report = Restaurant::findOrFail($id);
        $posms = Posm::where('status', 1)->orderBy('id', 'desc')->get();
        ...
        return view('backend.restaurant.add', compact('report', 'customers', 'customer', 'customerType', 'regions', 'outlets', 'posms'));

    }

    public function update(Request $request, $id)
    {
        $report = Restaurant::find($id);
        if (!$report) {
            return redirect()->route('restaurant.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('restaurant.index')->with('success', __('Report restaurant has been updated successfully.'));
    }
    ...
    public function export(Request $request)
    {
        ...
        if ($request->has('date1') && $request->has('date2') && $request->has('area_id')) {
            return Excel::download(
                new RestaurantExport($request->date1, $request->date2, $request->area_id, $request),
    ...
    public function getReports()
    {
        ...
        $query = Restaurant::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            Restaurant::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            Restaurant::whereNull('deleted_at')
