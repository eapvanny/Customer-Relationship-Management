...
use App\Exports\ExclusiveExport;
...
class ExclusiveController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Exclusive::with('user', 'CustomerProvince', 'region', 'outlet')->whereDate('created_at', today())->orderBy('id', 'desc');
        $user = auth()->user();
         ...
         * }
        */
        return view('backend.exclusive.index', compact('is_filter', 'full_name', 'reports', 'indexof', 'regions'));
        // dd('HI Wholesale');
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('exclusive.index')->with('success', __('Report exclusive has been created successfully.'));
    }
    ...
    */
    ...
    public function show($id)
    {
        $data['report'] = Exclusive::with('user', 'CustomerProvince', 'region', 'outlet', 'posm1', 'posm2', 'posm3')->findOrFail($id);
        return view('backend.exclusive.show', $data);
    }
    ...
    {
        $report = Exclusive::findOrFail($id);
        $posms = Posm::where('status', 1)->orderBy('id', 'desc')->get();
        ...
        return view('backend.exclusive.add', compact('report', 'customers', 'customer', 'customerType', 'regions', 'outlets', 'posms'));

    }

    public function update(Request $request, $id)
    {
        $report = Exclusive::find($id);
        if (!$report) {
            return redirect()->route('exclusive.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('exclusive.index')->with('success', __('Report exclusive has been updated successfully.'));
    }
    ...
    public function export(Request $request)
    {
        ...
        if ($request->has('date1') && $request->has('date2') && $request->has('area_id')) {
            return Excel::download(
                new ExclusiveExport($request->date1, $request->date2, $request->area_id, $request),
                'reports_exclusive_' . now()->format('Y_m_d_His') . '.xlsx'
            );
        } else {
            return Excel::download(
                new ExclusiveExport($request->date1, $request->date2, $request->area_id, $request),
    ...
    public function getReports()
    {
        ...
        $query = Exclusive::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            Exclusive::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            Exclusive::whereNull('deleted_at')
