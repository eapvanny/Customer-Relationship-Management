...
class SchoolimportController extends Controller
{
    ...
    {
            // dd('hi');
        $query = School_import::orderBy('id', 'desc');
        // dd($query);
        $user = auth()->user();
            ...
            return DataTables::of($reports)
               ...
               ->addColumn('outlet_id', function ($data) {
                ...
                ->addColumn('action', function ($data) {
                    $editRoute = route('school-import.edit', $data->id);
                    $deleteRoute = route('school-import.destroy', $data->id);
        ...
        return view('backend.school.import-list', compact('is_filter', 'full_name'));
        // dd('HI Wholesale');
    ...
    public function create()
    {
        ...
        return view('backend.school.take-photo', compact('customer', 'customers','report','customerType'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('school-import.index')->with('success', 'File imported successfully.');
    }
    ...
    {
        $report = School_import::with('user')->find($id);
        if (!$report) {
    ...
    {
        $report = School_import::find($id);
        if (!$report) {
            return redirect()->route('school-import-import.index');
        }
        ...
        // $customerType = AppHelper::CUSTOMER_TYPE;
        return view('backend.school.take-photo', compact('report'));
    }
    ...
    public function update(Request $request, $id)
    {
        $report = School_import::find($id);
        if (!$report) {
            return redirect()->route('school-import.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('school-import.index')->with('success', "Report has been updated!");
    }
    ...
    {
        // dd('HI Export');
        return Excel::download(new SchoolimportExport(), 'reports_school_dataimport' . now()->format('Y_m_d_His') . '.xlsx');
    }
    ...
    public function getReports()
    {
        ...
        $query = School_import::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            School_import::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            School_import::whereNull('deleted_at')
    ...
    */
    ...
    public function import(){
        ...
        return view('backend.school.import', compact('customer', 'customers','report','customerType'));
