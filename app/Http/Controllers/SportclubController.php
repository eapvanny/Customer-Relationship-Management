...
use App\Exports\SportclubExport;
...
class SportclubController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Sport_club::with('user', 'CustomerProvince', 'region', 'outlet')->whereDate('created_at', today())->orderBy('id', 'desc');
        $user = auth()->user();
         ...
         * }
        */
        return view('backend.sport-club.index', compact('is_filter', 'full_name', 'reports', 'indexof', 'regions'));
        // dd('HI Wholesale');
    ...
    public function create()
    {
        ...
        return view('backend.sport-club.add', compact('customer', 'customers', 'report', 'customerType', 'regions', 'outlets', 'posms'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('sportClub.index')->with('success', __('Report sport club has been created successfully.'));
    }
    ...
    {
        $report = Se_program::with('user', 'CustomerProvince', 'region', 'outlet')->find($id);
        if (!$report) {
    ...
    */
    ...
    public function show($id)
    {
        $data['report'] = Sport_club::with('user', 'CustomerProvince', 'region', 'outlet', 'posm1', 'posm2', 'posm3')->findOrFail($id);
        return view('backend.sport-club.show', $data);
    }
    ...
    {
        $report = Sport_club::findOrFail($id);
        $posms = Posm::where('status', 1)->orderBy('id', 'desc')->get();
        ...
        return view('backend.sport-club.add', compact('report', 'customers', 'customer', 'customerType', 'regions', 'outlets', 'posms'));

    }

    public function update(Request $request, $id)
    {
        $report = Sport_club::find($id);
        if (!$report) {
            return redirect()->route('sportClub.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('sportClub.index')->with('success', __('Report sport club has been updated successfully.'));
    }
    ...
    public function export(Request $request)
    {
        if ($request->has('date1') && $request->has('date2') && $request->has('area_id')) {
            return Excel::download(
                new SportclubExport($request->date1, $request->date2, $request->area_id, $request),
                'reports_restaurant_' . now()->format('Y_m_d_His') . '.xlsx'
            );
        } else {
            return Excel::download(
                new SportclubExport($request->date1, $request->date2, $request->area_id, $request),
    ...
    public function getReports()
    {
        ...
        $query = Sport_club::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            Sport_club::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            Sport_club::whereNull('deleted_at')
