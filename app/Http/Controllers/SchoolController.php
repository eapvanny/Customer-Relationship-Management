...
class SchoolController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = School::with('user', 'CustomerProvince', 'region', 'outlet')->whereDate('created_at', today())->orderBy('id', 'desc');
        $user = auth()->user();
         ...
         * }
        */
        return view('backend.school.index', compact('is_filter', 'full_name', 'reports', 'indexof', 'regions'));
        // dd('HI Wholesale');
    ...
    public function create()
    {
        ...
        return view('backend.school.add', compact('customer', 'customers', 'report', 'customerType', 'regions', 'outlets', 'posms'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('school.index')->with('success', __('Report School has been created successfully.'));
    }
    ...
    */
    ...
    public function show($id)
    {
        $data['report'] = School::with('user', 'CustomerProvince', 'region', 'outlet', 'posm1', 'posm2', 'posm3')->findOrFail($id);
        return view('backend.school.show', $data);
    }
    ...
    public function edit($id)
    {
        ...
        return view('backend.school.add', compact('report', 'customers', 'customer', 'customerType', 'regions', 'outlets', 'posms'));

    }

    public function update(Request $request, $id)
    {
        $report = School::find($id);
        if (!$report) {
            return redirect()->route('school.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('school.index')->with('success', __('Report School has been updated successfully.'));
    }
    ...
    public function export(Request $request)
    {
        ...
        if ($request->has('date1') && $request->has('date2') && $request->has('area_id')) {
            return Excel::download(
                new SchoolExport($request->date1, $request->date2, $request->area_id, $request),
                'reports_school_' . now()->format('Y_m_d_His') . '.xlsx'
            );
        } else {
            return Excel::download(
                new SchoolExport($request->date1, $request->date2, $request->area_id, $request),
    ...
    public function getReports()
    {
        ...
        $query = School::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            School::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            School::whereNull('deleted_at')
