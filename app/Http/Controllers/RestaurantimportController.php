...
use App\Exports\RestaurantimportExport;
use App\Models\User;
...
use App\Models\Restaurant_import;
...
class RestaurantimportController extends Controller
{
    /**
    ...
    {
            // dd('hi');
        $query = Restaurant_import::orderBy('id', 'desc');
        // dd($query);
        $user = auth()->user();
            ...
            return DataTables::of($reports)
               ...
               ->addColumn('outlet_id', function ($data) {
                ...
                ->addColumn('action', function ($data) {
                    $editRoute = route('restaurant-import.edit', $data->id);
                    $deleteRoute = route('restaurant-import.destroy', $data->id);
        ...
        return view('backend.restaurant.import-list', compact('is_filter', 'full_name'));
        // dd('HI Wholesale');
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('restaurant-import.index')->with('success', 'File imported successfully.');
    }
    ...
    {
        $report = Restaurant_import::with('user')->find($id);
        if (!$report) {
    ...
    {
        $report = Restaurant_import::find($id);
        if (!$report) {
            return redirect()->route('restaurant-import.index');
        }
        ...
        // $customerType = AppHelper::CUSTOMER_TYPE;
        return view('backend.restaurant.take-photo', compact('report'));
    }
    ...
    public function update(Request $request, $id)
    {
        $report = Restaurant_import::find($id);
        if (!$report) {
            return redirect()->route('restaurant-import.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('restaurant-import.index')->with('success', "Report has been updated!");
    }
    ...
    {
        // dd('HI Export');
        return Excel::download(new RestaurantimportExport(), 'reports_restaurant_dataimport' . now()->format('Y_m_d_His') . '.xlsx');
    }
    ...
    public function getReports()
    {
        ...
        $query = Restaurant_import::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            Restaurant_import::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            Restaurant_import::whereNull('deleted_at')
    ...
    */
    ...
    public function import(){
        ...
        return view('backend.restaurant.import', compact('customer', 'customers','report','customerType'));
