...
use App\Exports\ProvinceReportsExport;
...
class ProvinceReportController extends Controller
{
    ...
    {
        $this->middleware('type.permission:view daily sale province', ['only' => ['index']]);
        $this->middleware('type.permission:create daily sale province', ['only' => ['create', 'store']]);
        $this->middleware('type.permission:update daily sale province', ['only' => ['update', 'edit']]);
        $this->middleware('type.permission:delete daily sale province', ['only' => ['destroy']]);
    }
    public $indexof = 1;
    ...
    {
        $user = Auth::user();
        $query = ProvinceReport::with(['user','region', 'customer', 'customer.outlet']);

        $hasNoReports = !ProvinceReport::where('user_id', $user->id)
        ...
        if ($user) {
            ...
            if (!($userType == AppHelper::ALL || in_array($userRole, [
                ...
                $query->whereIn('province_reports.user_id', array_unique($userIds));
                $query->whereHas('user', function ($q) use ($allowedTypes) {
                    $q->whereIn('type', $allowedTypes);
                });
            }
        } else {
            $query->where('province_reports.id', 0);
        }
        ...
        $areas = Region::where('active_status', 1)
            ->select('region_name', 'rg_manager_kh', 'rg_manager_en', 'se_code', 'id')
            ->orderBy('region_name', 'asc')
            ->get()
            ->groupBy('rg_manager_kh')
            ->mapWithKeys(function ($group, $managerKh) {
                $label = $group->first()->region_name . ' (' .
                    (auth()->user()->user_lang == 'en'
                        ? $group->first()->rg_manager_en
                        : $group->first()->rg_manager_kh) . ')';

                return [
                    $label => $group->pluck('se_code', 'id')->toArray(),
                ];
            })
            ->toArray();
        ...
        if ($request->filled(['date1', 'date2'])) {
            ...
            $query->whereBetween('province_reports.date', [$startDate, $endDate]);
        }

        if ($request->filled('area_id')) {
            $is_filter = true;
            $query->where('province_reports.area_id', $request->area_id);
        }
            ...
            try {
                ...
                return DataTables::of($reports)
                    ->addColumn('area', function ($data) {
                        return $data->region ? $data->region->region_name ?? 'N/A' : 'N/A';
                    })
                    ->addColumn('outlet_id', function ($data) {
                        return $data->outlet ? $data->outlet->name ?? 'N/A' : 'N/A';
                    })
                    ...
                    ->addColumn('action', function ($data) {
                        ...
                        $edit = auth()->user()->can('update daily sale province')
                            ? '<a title="Edit" href="' . route('reports.edit', $data->id) . '" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i></a></span>'
                            : '';
                        return $show . ' ' . $edit;
                    })
                    ->rawColumns(['action'])
        ...
        return view('backend.province_report.list', compact('is_filter', 'areas'));
    }
    ...
    public function create()
    {
        ...
        $customerType = [];
        $depos = [];
        $posm = Posm::get()->pluck('name_kh', 'id')->toArray();
        $areas = Region::where('active_status', 1)
            ->select('region_name', 'rg_manager_kh', 'rg_manager_en', 'se_code', 'id')
            ->orderBy('region_name', 'asc')
            ->get()
        ...
        if ($areaId) {
            $customers = CustomerProvince::where('area_id', $areaId)->get(['id', 'name', 'depo_id']);
        }

        return view('backend.province_report.add', compact('customer', 'customers', 'report', 'customerType','areas','depos', 'posm'));
    }

    public function show($id)
    {
        $report = ProvinceReport::with(['user', 'customer','region'])->find($id);
        ...
        return response()->json([
            'report' => [
                ...
                'area' => $report->region->region_name ?? 'N/A',
                'outlet' => $report->outlet->name ?? 'N/A',
    ...
    public function getOutletsProvince(Request $request)
    {
        ...
        $query = Outlet::where('area_id', $areaId);
    ...
    public function getCustomersProvince(Request $request)
    {
        $areaId = $request->query('area_id');
    ...
    public function getCustomerTypeProvince(Request $request)
    {
    ...
    public function store(Request $request)
    {
        ...
        $prefix = Region::where('id', $areaName)->value('region_name');
        // Get the last report that starts with the current prefix
        $lastReport = ProvinceReport::orderBy('id', 'desc')->first();
        ...
        return redirect()->route('reports.index')->with('success', "Report has been created!");
    }

    public function edit($id)
    {
        $report = ProvinceReport::with('customer')->findOrFail($id);
        ...
        $depos = Outlet::where('area_id', $report->area_id)
            ->pluck('name', 'id')
            ->toArray();

        // Fetch customers for the report's area_id and outlet_id (depo_id)
        $customers = CustomerProvince::where('area_id', $report->area_id)
        ...
        return view('backend.province_report.add', compact('report', 'areas', 'depos', 'customers', 'customerType', 'posm'));
    }

    public function update(Request $request, $id)
    {
        $report = ProvinceReport::find($id);
        if (!$report) {
        ...
        if (!$report->so_number) {
            ...
            $prefix = Region::where('id', $areaName)->value('region_name');
            // Get the last report that starts with the current prefix
            $lastNumberReport = ProvinceReport::where('so_number', 'like', $prefix . '-%')
                ->orderBy('id', 'desc')
                ->first();
        ...
        return redirect()->route('reports.index')->with('success', 'Report has been updated!');
    }
    ...
    // Fetch recent unseen reports for notifications
    public function getReports()
    {
        ...
        $query = ProvinceReport::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(
            new ProvinceReportsExport($date1, $date2, $area_id),
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            ProvinceReport::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            ProvinceReport::whereNull('deleted_at')
