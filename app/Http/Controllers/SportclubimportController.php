...
use App\Exports\SchoolimportExport;
use App\Exports\SportclubimportExport;
use App\Imports\SubwholesaleImport;
use App\Http\Controllers\Controller;
use App\Imports\SportclubImport;
...
class SportclubimportController extends Controller
{
    /**
    ...
    {
            // dd('hi');
        $query = Sport_club_import::orderBy('id', 'desc');
        // dd($query);
        $user = auth()->user();
            ...
            return DataTables::of($reports)
               ...
               ->addColumn('outlet_id', function ($data) {
                ...
                ->addColumn('action', function ($data) {
                    $editRoute = route('sport-club-import.edit', $data->id);
                    $deleteRoute = route('sport-club-import.destroy', $data->id);
        ...
        return view('backend.sport-club.import-list', compact('is_filter', 'full_name'));
        // dd('HI Wholesale');
        // return view('backend.retail-import.index');
    }
    ...
    public function create()
    {
        ...
        return view('backend.sport-club.take-photo', compact('customer', 'customers','report','customerType'));
    }
    ...
    public function store(Request $request)
    {
        ...
        // exit;
        // Import the file
        Excel::import(new SportclubImport, $file);
        // dd('File imported successfully.');
        return redirect()->route('sport-club-import.index')->with('success', 'File imported successfully.');
    }
    ...
    {
        $report = Sport_club_import::with('user')->find($id);
        if (!$report) {
    ...
    {
        $report = Sport_club_import::find($id);
        if (!$report) {
            return redirect()->route('sport-club-import.index');
        }
        ...
        // $customerType = AppHelper::CUSTOMER_TYPE;
        return view('backend.sport-club.take-photo', compact('report'));
    }
    ...
    public function update(Request $request, $id)
    {
        $report = Sport_club_import::find($id);
        if (!$report) {
            return redirect()->route('sport-club-import.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('sport-club-import.index')->with('success', "Report has been updated!");
    }
    ...
    {
        // dd('HI Export');
        return Excel::download(new SportclubimportExport(), 'reports_sport_club_dataimport' . now()->format('Y_m_d_His') . '.xlsx');
    }
    ...
    public function getReports()
    {
        ...
        $query = Sport_club_import::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            Sport_club_import::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            Sport_club_import::whereNull('deleted_at')
    ...
    public function import(){
        ...
        return view('backend.retail-import.import', compact('customer', 'customers','report','customerType'));
        // return view('backend.retail-import.import', compact('report', 'customers'));
    }
    ...
    */
    ...
    public function import(){
        ...
        return view('backend.sport-club.import', compact('customer', 'customers','report','customerType'));
