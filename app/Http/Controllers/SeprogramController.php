...
use App\Exports\SeprogramExport;
...
class seprogramController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        $query = Se_program::with('user', 'CustomerProvince', 'region', 'outlet')->whereDate('created_at', today())->orderBy('id', 'desc');
        $user = auth()->user();
         ...
         * }
        */
        return view('backend.se.index', compact('is_filter', 'full_name', 'reports', 'indexof', 'regions'));
        // dd('HI Wholesale');
    ...
    public function create()
    {
        ...
        return view('backend.se.add', compact('customer', 'customers', 'report', 'customerType', 'regions', 'outlets', 'posms'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('se.index')->with('success', __('Report SE Program has been created successfully.'));
    }
    ...
    */
    ...
    public function show($id)
    {
        $data['report'] = Se_program::with('user', 'CustomerProvince', 'region', 'outlet', 'posm1', 'posm2', 'posm3')->findOrFail($id);
        return view('backend.se.show', $data);
    }
    ...
    {
        $report = Se_program::findOrFail($id);
        $posms = Posm::where('status', 1)->orderBy('id', 'desc')->get();
        ...
        return view('backend.se.add', compact('report', 'customers', 'customer', 'customerType', 'regions', 'outlets', 'posms'));
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$report) {
            return redirect()->route('se.index')->with('error', 'Report not found!');
        }
        ...
        return redirect()->route('se.index')->with('success', __('Report ASM Program has been updated successfully.'));
    }
    ...
    public function export(Request $request)
    {
        ...
        if ($request->has('date1') && $request->has('date2') && $request->has('area_id')) {
            return Excel::download(
                new SeprogramExport($request->date1, $request->date2, $request->area_id, $request),
                'reports_semprogram_' . now()->format('Y_m_d_His') . '.xlsx'
            );
        } else {
            return Excel::download(
                new SeprogramExport($request->date1, $request->date2, $request->area_id, $request),
    ...
    public function getReports()
    {
        ...
        $query = Se_program::with('user')->whereNull('deleted_at')->where('is_seen', false);
    ...
    public function markAsSeen()
    {
        ...
        if (in_array($user->role_id, [AppHelper::USER_SUPER_ADMIN, AppHelper::USER_ADMIN])) {
            Se_program::whereNull('deleted_at')->update(['is_seen' => true]);
        } elseif ($user->role_id == AppHelper::USER_MANAGER) {
            Se_program::whereNull('deleted_at')
