...
class SubwholesaleExport implements FromView
{
    ...
    {
        $user = Auth::user();
        $query = Display_subwholesale::with('user')->orderBy('id', 'desc');
        ...
        if (!$user) {
            ...
            if (!($userType == AppHelper::ALL || in_array($userRole, [
                ...
                $query
                    ->whereIn('apply_user', array_unique($userIds))
        ...
        $reports = $query->get();
        $title = 'Sub-Wholesale Export';
        return view('exports.subwholesale-export', compact('is_filter', 'full_name', 'reports', 'title'));
    }
}
