...
@endphp
<h5>ទាញយករបាយការណ៍របស់ផ្នែក SE (Exclusive/sport club)</h5>
<table border="1">
    <thead>
        <tr>
            <th>{{__('Staff ID')}}</th>
    ...
    <tbody>
        @foreach ($rows as $row)
            <tr>
                ...
                <td>{{ __(AppHelper::getAreaName($row->area_id)) ?? 'N/A' }}</td>
                <td>{{ $row->outlet_id ? $row->customer->outlet : 'N/A'; }}</td>
                ...
                {{-- <td>{{ isset(AppHelper::CUSTOMER_TYPE[$row->customer_type]) ? __(AppHelper::CUSTOMER_TYPE[$row->customer_type]) : __('N/A') }}</td>
                 --}}
                <td>
                    {{ __(AppHelper::CUSTOMER_TYPE[$row->customer_type] ?? 'N/A') }}
                </td>
                <td>{{ $row->{'250_ml'} ?? 0 }}</td>
                <td>{{ $row->{'350_ml'} ?? 0 }}</td>
                <td>{{ $row->{'600_ml'} ?? 0 }}</td>
