...
@section('pageContent')
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href="{{ URL::route('reports.index') }}"> {{ __('Reports') }} </a></li>
            <li class="active">
                @if ($report)
                    {{ __('Update') }}
                @else
    ...
    <section class="content">
        <form novalidate id="entryForm"
            action="@if ($report) {{ URL::Route('reports.update', $report->id) }} @else {{ URL::Route('reports.store') }} @endif"
                ...
                <div class="col-md-12">
                    <div class="wrap-outter-header-title">
                        ...
                        <div class="box-tools pull-right">
                            <a href="{{ URL::route('reports.index') }}" class="btn btn-default">{{ __('Cancel') }}</a>
                    ...
                    <div class="row">
                        <div class="col-md-6 col-xl-6">
                            <div class="form-group has-feedback">
                                <label for="area">{{ __('Regions') }} <span class="text-danger">*</span></label>
                                <select name="area" id="area" class="form-control select2" required>
    <option value="">{{ __('Select region') }}</option>
    @foreach($areas as $rgManagerKh => $regionGroup)
        <optgroup label="
            {{ $regionGroup->first()->region_name }}
            (
        ...
        ">
            @foreach($regionGroup as $region)
                <option value="{{ $region->id }}"
                    @if (old('area', optional($report)->area_id) == $region->id) selected @endif>
                    {{ $region->se_code }}
                </option>
            @endforeach
        </optgroup>
                                                    ...
                                                    <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                                                        <div class="form-group has-feedback">
                                                            ...
                                                            {!! Form::select(
                                                                'posm',
                                                                $posm,
                                                                old('posm', optional($report)->posm), // selected value
                                                                [
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#area').on('change', function() {
                ...
                if (areaId) {
                    $.ajax({
                        url: "{{ route('customers.outlet-pro') }}",
                        method: "GET",
                        data: {
                            area_id: areaId
                        },
            ...
            $('#customer_id').on('change', function() {
                ...
                if (areaId && outletId && customerId) {
                    $.ajax({
                        url: '{{ route('customers.getCustomerType-pro') }}',
                        type: 'GET',
                        data: {
