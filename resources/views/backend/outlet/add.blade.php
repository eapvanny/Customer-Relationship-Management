...
@section('pageContent')
    ...
    <section class="content-header">
        <ol class="breadcrumb">
            ...
            <li><a href="{{ URL::route('outlet.index') }}"> {{ __('Depot') }} </a></li>
            <li class="active">
                @if ($outlet)
                    {{ __('Update') }}
                @else
                    ...
                    <div class="wrap-outter-header-title">
                        ...
                        <div class="action-btn-top none_fly_action_btn">
                            ...
                            <button type="submit" class="submitClick btn btn-info pull-right text-white">
                                <i class="fa @if ($outlet) fa-refresh @else fa-check-circle @endif"></i>
                                @if ($outlet)
                                    {{ __('Update') }}
                                @else
                                    {{ __('Save') }}
                    ...
                    <div class="box-body">
                        <div class="row">
                                            ...
                                            <optgroup label="{{ $regionGroup->first()->region_name }} (@if(auth()->user()->user_lang == 'en') {{ $regionGroup->first()->rg_manager_en }} @else {{ $regionGroup->first()->rg_manager_kh }} @endif)">
                                                @foreach($regionGroup as $region)
                                                    <option value="{{ $region->id }}"
                                                        @if ($outlet)
                                                            {{ $outlet->area_id == $region->id ? 'selected' : ''}}
                                                        @endif
                                                        @if(old('area'))
                            ...
                            <div class="col-lg-6 col-md-6 col-xl-6">
                                <div class="form-group has-feedback">
                                    <label for="name">{{ __('Depot Name') }} <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="name" class="form-control" placeholder="{{ __('Depot Name') }}"
                            ...
                            <div class="col-lg-6 col-md-6 col-xl-4">
                                <div class="form-group has-feedback">
                                    <label for="code">{{ __('Depot code') }} <span
                                            class="text-danger">*</span></label>
                                    <input type="text" name="code" class="form-control" placeholder="{{ __('Depot code') }}"
                            ...
                            <div class="col-lg-6 col-md-6 col-xl-4">
                                <div class="form-group has-feedback">
                                    ...
                                    <input type="tel" class="form-control" name="phone"
                                        ...
                                        value="@if($outlet){{ $outlet->phone }}@else{{ old('phone') }}@endif">
                                         <span class="fa fa-user form-control-feedback"></span>
                            ...
                            <div class="form-group has-feedback">
                                ...
                                {{-- {!! Form::select('customer_type', $customerType, old('customer_type', optional($outlet)->customer_type), [
                                ...
                                <select name="customer_type" id="customer_type" class="form-control select2" required>
                                    ...
                                    <option value="depot" selected>{{__("Depot")}}</option>
                                </select>
                            ...
                            <div class="col-xl-12 col-lg-12 col-md-12">
                                <fieldset>
                                    <legend class="fs-6">{{ __("Depot's photo") }} <span class="text-danger">*</span></legend>
                                    <div class="row">
                                        <div class="form-group has-feedback">
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="form-group has-feedback position-relative">
                                                        ...
                                                        <fieldset id="outlet-photo-upload"
                                                            ...
                                                            <img class="rounded mx-auto d-block @if (!old('old_outlet_photo') && !isset($outlet->outlet_photo)) d-none @endif z-1"
                                                                id="outlet-photo-preview" name="old_outlet_photo"
                                                                src="@if (isset($outlet->outlet_photo)) {{ asset('storage/' . $outlet->outlet_photo) }} @else {{ old('old_outlet_photo') }} @endif"
                                                                alt="outlet-photo">
                                                            <input type="hidden" id="outlet-img-preview"
                                                                name="old_outlet_photo"
                                                                value="@if (isset($outlet->outlet_photo)) {{ $outlet->outlet_photo }} @endif">
                                                            <div class="d-flex align-items-center justify-content-center bg-transparent z-2 @if (!old('outlet-img-preview') && !isset($outlet->outlet_photo)) opacity-100 @else opacity-25 @endif"
                                                            ...
                                                            <label id="outlet-camera-label"
                                                                ...
                                                                @if (isset($outlet->outlet_photo) || old('outlet-img-preview'))
                                    ...
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            ...
                                            <input type="text" class="form-control" name="latitude" id="latitude" placeholder="{{ __('Latitude') }}"
                                                value="{{ $outlet ? $outlet->latitude : old('latitude') }}"
                                                readonly required>
                                                <span class="fa fa-info form-control-feedback"></span>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            ...
                                            <input type="text" class="form-control" name="longitude" id="longitude" placeholder="{{ __('Longitude') }}"
                                                value="{{ $outlet ? $outlet->longitude : old('longitude') }}"
                                                readonly required>
                                                <span class="fa fa-info form-control-feedback"></span>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            <label for="city">{{ __('Address') }}<span
                                                    class="text-danger">*</span></label>
                                            <textarea class="form-control" name="city" id="city" cols="30" rows="1" placeholder="{{ __('Address') }}" readonly required>{{ $outlet ? $outlet->city : old('city') }}</textarea>
                                            <span class="fa fa-info form-control-feedback"></span>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            ...
                                            <input type="text" class="form-control" name="country" id="country" placeholder="{{ __('Country') }}"
                                                value="{{ $outlet ? $outlet->country : old('country') }}" readonly
                                                required>
                                                <span class="fa fa-info form-control-feedback"></span>
...
@section('extraScript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // let selected = "{{ old('outlet_id', $outlet->depo_id ?? '') }}";
            // $.each(response, function (id, name) {
            ...
            @if ($outlet && $outlet->latitude && $outlet->longitude)
                window.onload = function() {
                    initMap({{ $outlet->latitude }}, {{ $outlet->longitude }});
                }
            @else
                window.onload = function() {
                    initMap(0, 0);
