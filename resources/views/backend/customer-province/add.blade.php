...
@section('pageContent')
    ...
    <section class="content">
        <form id="entryForm"
            action="@if ($customer_province) {{ URL::Route('cp.update', $customer_province->id) }} @else {{ URL::Route('cp.store') }} @endif"
                ...
                <div class="box box-info">
                    <div class="box-body">
                        <div class="row">
                                            ...
                                            <optgroup label="{{ $regionGroup->first()->region_name }} (@if(auth()->user()->user_lang == 'en') {{ $regionGroup->first()->rg_manager_en }} @else {{ $regionGroup->first()->rg_manager_kh }} @endif)">
                                                @foreach($regionGroup as $region)
                                                    <option value="{{ $region->id }}"
                                                        @if ($customer_province)
                                                            {{ $customer_province->area_id == $region->id ? 'selected' : ''}}
                                                        @endif
                                                        @if(old('area'))
                                                        {{ (old('area')  == $region->id ? 'selected' : '') }}
                                                        @endif>
                                                        {{ $region->se_code }}
                                                    </option>
                                                @endforeach
                                    ...
                                    <select name="depo_id" class="form-control select2" id="depo_id" required>
                                        ...
                                        @if(old('depo_id') || $errors->first('depo_id') || $customer_province)
                                            @foreach($outlets as $outlet)
                                                <option value="{{ $outlet->id }}" @if (old('depo_id') == $outlet->id) selected @else {{ @$customer_province->depo_id == $outlet->id }} selected @endif>
                                                    {{ $outlet->name }}
                                                </option>
                                            @endforeach
                                        @endif
                            ...
                            <div class="col-lg-6 col-md-6 col-xl-4">
                                <div class="form-group has-feedback">
                                    ...
                                    <input type="text" name="name" class="form-control" placeholder="{{ __('Customer Name') }}"
                                        value="{{ old('name', $customer_province->name ?? '') }}" required>
                            ...
                            <div class="col-lg-6 col-md-6 col-xl-4">
                                <div class="form-group has-feedback">
                                    ...
                                    <input type="tel" class="form-control" name="phone"
                                        ...
                                        value="@if($customer_province){{ $customer_province->phone }}@else{{ old('phone') }}@endif">
                                         <span class="fa fa-user form-control-feedback"></span>
                            ...
                            <div class="form-group has-feedback">
                                ...
                                {!! Form::select('customer_type', $customerType, old('customer_type', optional($customer_province)->customer_type), [
                            ...
                            <div class="col-xl-12 col-lg-12 col-md-12">
                                <fieldset>
                                    <legend class="fs-6">{{ __("Customer's shop photo") }} <span class="text-danger">*</span></legend>
                                    <div class="row">
                                        <div class="form-group has-feedback">
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <div class="form-group has-feedback position-relative">
                                                        ...
                                                        <fieldset id="outlet-photo-upload"
                                                            ...
                                                            <img class="rounded mx-auto d-block @if (!old('old_outlet_photo') && !isset($customer_province->outlet_photo)) d-none @endif z-1"
                                                                id="outlet-photo-preview" name="old_outlet_photo"
                                                                src="@if (isset($customer_province->outlet_photo)) {{ asset('storage/' . $customer_province->outlet_photo) }} @else {{ old('old_outlet_photo') }} @endif"
                                                                alt="outlet-photo">
                                                            <input type="hidden" id="outlet-img-preview"
                                                                name="old_outlet_photo"
                                                                value="@if (isset($customer_province->outlet_photo)) {{ $customer_province->outlet_photo }} @endif">
                                                            <div class="d-flex align-items-center justify-content-center bg-transparent z-2 @if (!old('outlet-img-preview') && !isset($customer_province->outlet_photo)) opacity-100 @else opacity-25 @endif"
                                                            ...
                                                            <label id="outlet-camera-label"
                                                                ...
                                                                @if (isset($customer_province->outlet_photo) || old('outlet-img-preview'))
                        ...
                        <div class="col-lg-12 col-md-12 col-xl-12">
                            <fieldset>
                                <legend class="fs-6">{{ __('Customer location') }}</legend>
                                <div class="row">
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            <label for="latitude">{{ __('Latitude') }}<span
                                                    class="text-danger">*</span></label>
                                            <input type="text" class="form-control" name="latitude" id="latitude" placeholder="{{ __('Latitude') }}"
                                                value="{{ isset($customer_province) ? $customer_province->latitude : old('latitude') }}"
                                                readonly required>
                                                <span class="fa fa-info form-control-feedback"></span>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            <label for="longitude">{{ __('Longitude') }}<span
                                                    class="text-danger">*</span></label>
                                            <input type="text" class="form-control" name="longitude" id="longitude" placeholder="{{ __('Longitude') }}"
                                                value="{{ isset($customer_province) ? $customer_province->longitude : old('longitude') }}"
                                                readonly required>
                                                <span class="fa fa-info form-control-feedback"></span>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            <label for="city">{{ __('Address') }}<span
                                                    class="text-danger">*</span></label>
                                            <textarea class="form-control" name="city" id="city" cols="30" rows="1" placeholder="{{ __('Address') }}" readonly required>{{ isset($customer_province) ? $customer_province->city : old('city') }}</textarea>
                                            <span class="fa fa-info form-control-feedback"></span>
                                    ...
                                    <div class="col-lg-6 col-md-6 col-xl-6">
                                        <div class="form-group has-feedback">
                                            ...
                                            <input type="text" class="form-control" name="country" id="country" placeholder="{{ __('Country') }}"
                                                value="{{ isset($customer_province) ? $customer_province->country : old('country') }}" readonly
                                                required>
                                                <span class="fa fa-info form-control-feedback"></span>
...
@section('extraScript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // let selected = "{{ old('outlet_id', $customer_province->depo_id ?? '') }}";
            // $.each(response, function (id, name) {
            ...
            @if ($customer_province && $customer_province->latitude && $customer_province->longitude)
                window.onload = function() {
                    initMap({{ $customer_province->latitude }}, {{ $customer_province->longitude }});
                }
            @else
                window.onload = function() {
                    initMap(0, 0);
           ...
           // Handle form submission for Save and Save & Add New
            ...
            @if ($customer_province && $customer_province->outlet_photo)
